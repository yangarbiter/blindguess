...
def adversarial_training(X, y, eps, sep_measure, model, n_iters, attack_model):
    for _ in range(n_iters):
        clf = deepcopy(model)
        clf.fit(X, y)
        advX = attack_model(X, y, clf).perturb(X, y=y, eps=eps)
...
def get_aug_data(model, X, y, eps):
    ...
    elif model.train_type == 'boundaryAP':
        assert False
        if len(np.unique(y)) != 2:
            raise ValueError("Can only deal with number of classes = 2"
                             "got %d", len(np.unique(y)))

        assert model is not None, "model cannot be None for boundaryAP"
